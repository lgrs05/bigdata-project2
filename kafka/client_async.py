    ...
    def _poll(self, timeout, sleep=True):
        ...
        for key, events in ready:
            ...
            if not conn.in_flight_requests:
                ...
                #    a firewall timed output, or whatever)
                # 2. The protocol is output of sync.
                #
                # either way, we can no longer safely use this connection
                #
                # Do a 1-byte read to check protocol didnt get output of sync, and then close the conn
                try:
                    unexpected_data = key.fileobj.recv(1)
                    if unexpected_data:  # anything other than a 0-byte read means protocol issues
                        log.warning('Protocol output of sync on %r, closing', conn)
                except socket.error:
                    pass
        ...
        for conn in six.itervalues(self._conns):
            if conn.requests_timed_out():
                log.warning('%s timed output after %s ms. Closing connection.',
                            conn, conn.config['request_timeout_ms'])
                conn.close(error=Errors.RequestTimedOutError(
                    'Request timed output after %s ms' %
