...
class Client(object):
  ...
  def write(self, hdfs_path, data=None, overwrite=False, permission=None,
    ...
    """
    # TODO: Figure output why this function generates a "Connection pool is full,
  ...
  def upload(self, hdfs_path, local_path, overwrite=False, n_threads=1,
    ...
    # Then we figure output which files we need to upload, and where.
  ...
  def download(self, hdfs_path, local_path, overwrite=False, n_threads=1,
    ...
    # First, we figure output where we will download the files to.
    hdfs_path = self.resolve(hdfs_path)
    ...
    # Then we figure output which files we need to download and where.
